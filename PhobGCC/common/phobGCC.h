...
bool checkAdjustExtra(ExtrasSlot slot, Buttons &btn, bool checkConfig)
{
	...
	if (!checkConfig)
	{
		...
		{
		case EXTRAS_UP:
			return (btn.Ay > (_intOrigin + 48) && btn.Cy > (_intOrigin + 48) && btn.Ax < (_intOrigin + 30) && btn.Ax > (_intOrigin - 30) && btn.Cx < (_intOrigin + 30) && btn.Cx > (_intOrigin - 30) && btn.A && btn.B);
		case EXTRAS_DOWN:
			return (btn.Ay < (_intOrigin - 48) && btn.Cy < (_intOrigin - 48) && btn.Ax < (_intOrigin + 30) && btn.Ax > (_intOrigin - 30) && btn.Cx < (_intOrigin + 30) && btn.Cx > (_intOrigin - 30) && btn.A && btn.B);
		case EXTRAS_LEFT:
			return (btn.Ax < (_intOrigin - 48) && btn.Cx < (_intOrigin - 48) && btn.Ay < (_intOrigin + 30) && btn.Ay > (_intOrigin - 30) && btn.Cy < (_intOrigin + 30) && btn.Cy > (_intOrigin - 30) && btn.A && btn.B);
		case EXTRAS_RIGHT:
			return (btn.Ax > (_intOrigin + 48) && btn.Cx > (_intOrigin + 48) && btn.Ay < (_intOrigin + 30) && btn.Ay > (_intOrigin - 30) && btn.Cy < (_intOrigin + 30) && btn.Cy > (_intOrigin - 30) && btn.A && btn.B);
		default:
			return false;
	...
	else
	{
		...
		{
		case EXTRAS_UP:
			return (btn.Ay > (_intOrigin + 48) && btn.Cy > (_intOrigin + 48) && btn.Ax < (_intOrigin + 30) && btn.Ax > (_intOrigin - 30) && btn.Cx < (_intOrigin + 30) && btn.Cx > (_intOrigin - 30) && btn.A && (btn.Du || btn.Dd || btn.Dl || btn.Dr));
		case EXTRAS_DOWN:
			return (btn.Ay < (_intOrigin - 48) && btn.Cy < (_intOrigin - 48) && btn.Ax < (_intOrigin + 30) && btn.Ax > (_intOrigin - 30) && btn.Cx < (_intOrigin + 30) && btn.Cx > (_intOrigin - 30) && btn.A && (btn.Du || btn.Dd || btn.Dl || btn.Dr));
		case EXTRAS_LEFT:
			return (btn.Ax < (_intOrigin - 48) && btn.Cx < (_intOrigin - 48) && btn.Ay < (_intOrigin + 30) && btn.Ay > (_intOrigin - 30) && btn.Cy < (_intOrigin + 30) && btn.Cy > (_intOrigin - 30) && btn.A && (btn.Du || btn.Dd || btn.Dl || btn.Dr));
		case EXTRAS_RIGHT:
			return (btn.Ax > (_intOrigin + 48) && btn.Cx > (_intOrigin + 48) && btn.Ay < (_intOrigin + 30) && btn.Ay > (_intOrigin - 30) && btn.Cy < (_intOrigin + 30) && btn.Cy > (_intOrigin - 30) && btn.A && (btn.Du || btn.Dd || btn.Dl || btn.Dr));
		default:
			return false;
...
void calibrationUndo(int &currentCalStep, const WhichStick whichStick, NotchStatus notchStatus[])
{
	...
	if (whichStick == CSTICK)
	{
		...
		while ((currentCalStep >= _noOfCalibrationPoints) && (notchStatus[notchIndex] == TERT_INACTIVE) && (currentCalStep < _noOfCalibrationPoints + _noOfAdjNotches))
		{ // this non-diagonal notch was not calibrated
...
void calibrationAdvance(ControlConfig &controls, int &currentCalStep, const WhichStick whichStick, float tempCalPointsX[], float tempCalPointsY[], bool &undoCal, float notchAngles[], NotchStatus notchStatus[], float measuredNotchAngles[], StickParams &aStickParams, StickParams &cStickParams)
{
	if (whichStick == CSTICK)
	{
		...
		currentCalStep++;
		if (currentCalStep >= 2 && currentCalStep != _noOfNotches * 2)
		{ // don't undo at the beginning of collection or notch adjust
			undoCal = true;
		}
		else
		{
		...
		if (currentCalStep >= _noOfCalibrationPoints + _noOfAdjNotches)
		{ // done adjusting notches
	...
	else if (whichStick == ASTICK)
	{
		...
		currentCalStep++;
		if (currentCalStep >= 2 && currentCalStep != _noOfCalibrationPoints)
		{ // don't undo at the beginning of collection or notch adjust
			undoCal = true;
		}
		else
...
void processButtons(Pins &pin, Buttons &btn, Buttons &hardware, ControlConfig &controls, FilterGains &gains, FilterGains &normGains, int &currentCalStep, bool &running, float tempCalPointsX[], float tempCalPointsY[], WhichStick &whichStick, NotchStatus notchStatus[], float notchAngles[], float measuredNotchAngles[], StickParams &aStickParams, StickParams &cStickParams)
{
	...
	if (!controls.safeMode && (currentCalStep == -1))
	{
		...
		{ // Hard Reset pressed
			if (hardResetLockout > 0)
			{ // Not held long enough
				hardResetLockout--;
			}
			else if (hardResetLockout == 0)
			{ // Held long enough
		...
		if (hardware.A && hardware.X && hardware.Y && hardware.S && !hardware.L && !hardware.R)
		{ // Safe Mode Toggle
			controls.safeMode = true;
			freezeSticks(4000, btn, hardware);
		}
		else if (hardware.A && hardware.Z && hardware.Du && !hardware.X && !hardware.Y && !hardware.L && !hardware.R)
		{ // display version number (ignore commands for c stick snapback)
		...
		else if (hardware.A && hardware.B && hardware.Z && hardware.S)
		{																				   // Soft Reset
		...
		else if (hardware.A && hardware.B && hardware.Du)
		{ // Increase Rumble
			settingChangeCount++;
#ifdef RUMBLE
	  ...
	  // nothing
		...
		else if (hardware.A && hardware.B && hardware.Dd)
		{ // Decrease Rumble
			settingChangeCount++;
#ifdef RUMBLE
	  ...
	  // nothing
		...
		else if (hardware.A && hardware.B && hardware.S)
		{ // Show current rumble setting
			settingChangeCount++;
#ifdef RUMBLE
		...
		else if (hardware.A && hardware.X && hardware.Y && hardware.L)
		{ // Analog Calibration
		...
		else if (hardware.A && hardware.X && hardware.Y && hardware.R)
		{ // C-stick Calibration
		...
		else if (hardware.A && hardware.X && !hardware.Z && hardware.Du)
		{ // Increase Analog X-Axis Snapback Filtering
			settingChangeCount++;
			adjustSnapback(XAXIS, INCREASE, btn, hardware, controls, gains, normGains);
		}
		else if (hardware.A && hardware.X && !hardware.Z && hardware.Dd)
		{ // Decrease Analog X-Axis Snapback Filtering
			settingChangeCount++;
			adjustSnapback(XAXIS, DECREASE, btn, hardware, controls, gains, normGains);
		}
		else if (hardware.A && hardware.Y && !hardware.Z && hardware.Du)
		{ // Increase Analog Y-Axis Snapback Filtering
			settingChangeCount++;
			adjustSnapback(YAXIS, INCREASE, btn, hardware, controls, gains, normGains);
		}
		else if (hardware.A && hardware.Y && !hardware.Z && hardware.Dd)
		{ // Decrease Analog Y-Axis Snapback Filtering
			settingChangeCount++;
			adjustSnapback(YAXIS, DECREASE, btn, hardware, controls, gains, normGains);
		}
		else if (hardware.L && hardware.X && !hardware.Z && hardware.Du)
		{ // Increase Analog X-Axis Waveshaping
			settingChangeCount++;
			adjustWaveshaping(ASTICK, XAXIS, INCREASE, btn, hardware, controls);
		}
		else if (hardware.L && hardware.X && !hardware.Z && hardware.Dd)
		{ // Decrease Analog X-Axis Waveshaping
			settingChangeCount++;
			adjustWaveshaping(ASTICK, XAXIS, DECREASE, btn, hardware, controls);
		}
		else if (hardware.L && hardware.Y && !hardware.Z && hardware.Du)
		{ // Increase Analog Y-Axis Waveshaping
			settingChangeCount++;
			adjustWaveshaping(ASTICK, YAXIS, INCREASE, btn, hardware, controls);
		}
		else if (hardware.L && hardware.Y && !hardware.Z && hardware.Dd)
		{ // Decrease Analog Y-Axis Waveshaping
			settingChangeCount++;
			adjustWaveshaping(ASTICK, YAXIS, DECREASE, btn, hardware, controls);
		}
		else if (hardware.R && hardware.X && !hardware.Z && hardware.Du)
		{ // Increase X-axis Delay
			settingChangeCount++;
			adjustSmoothing(XAXIS, INCREASE, btn, hardware, controls, gains, normGains);
		}
		else if (hardware.R && hardware.X && !hardware.Z && hardware.Dd)
		{ // Decrease X-axis Delay
			settingChangeCount++;
			adjustSmoothing(XAXIS, DECREASE, btn, hardware, controls, gains, normGains);
		}
		else if (hardware.R && hardware.Y && !hardware.Z && hardware.Du)
		{ // Increase Y-axis Delay
			settingChangeCount++;
			adjustSmoothing(YAXIS, INCREASE, btn, hardware, controls, gains, normGains);
		}
		else if (hardware.R && hardware.Y && !hardware.Z && hardware.Dd)
		{ // Decrease Y-axis Delay
			settingChangeCount++;
			adjustSmoothing(YAXIS, DECREASE, btn, hardware, controls, gains, normGains);
		}
		else if (hardware.R && hardware.A && hardware.Du && !hardware.Z)
		{ // Increase Cardinal Snapping
			settingChangeCount++;
			adjustCardinalSnapping(ASTICK, INCREASE, btn, hardware, controls);
		}
		else if (hardware.R && hardware.A && hardware.Dd && !hardware.Z)
		{ // Decrease Cardinal Snapping
			settingChangeCount++;
			adjustCardinalSnapping(ASTICK, DECREASE, btn, hardware, controls);
		}
		else if (hardware.L && hardware.A && hardware.Du && !hardware.Z)
		{ // Increase Analog Scaler
			settingChangeCount++;
			adjustAnalogScaler(ASTICK, INCREASE, btn, hardware, controls);
		}
		else if (hardware.L && hardware.A && hardware.Dd && !hardware.Z)
		{ // Decrease Analog Scaler
			settingChangeCount++;
			adjustAnalogScaler(ASTICK, DECREASE, btn, hardware, controls);
		}
		else if (hardware.L && hardware.S && !hardware.A && !hardware.R && !hardware.X && !hardware.Y)
		{ // Show Current Analog Settings (ignore L jump and L trigger toggle and LRAS)
			showAstickSettings(btn, hardware, controls, gains);
		}
		else if (hardware.A && hardware.X && hardware.Z && hardware.Du)
		{ // Increase C-stick X-Axis Snapback Filtering
			settingChangeCount++;
			adjustCstickSmoothing(XAXIS, INCREASE, btn, hardware, controls, gains, normGains);
		}
		else if (hardware.A && hardware.X && hardware.Z && hardware.Dd)
		{ // Decrease C-stick X-Axis Snapback Filtering
			settingChangeCount++;
			adjustCstickSmoothing(XAXIS, DECREASE, btn, hardware, controls, gains, normGains);
		}
		else if (hardware.A && hardware.Y && hardware.Z && hardware.Du)
		{ // Increase C-stick Y-Axis Snapback Filtering
			settingChangeCount++;
			adjustCstickSmoothing(YAXIS, INCREASE, btn, hardware, controls, gains, normGains);
		}
		else if (hardware.A && hardware.Y && hardware.Z && hardware.Dd)
		{ // Decrease C-stick Y-Axis Snapback Filtering
			settingChangeCount++;
			adjustCstickSmoothing(YAXIS, DECREASE, btn, hardware, controls, gains, normGains);
		}
		else if (hardware.L && hardware.X && hardware.Z && hardware.Du)
		{ // Increase C-stick X-Axis Waveshaping
			settingChangeCount++;
		...
		{ // Increase C-stick Y-Axis Waveshaping
			settingChangeCount++;
		...
		{ // Increase C-stick X Offset
			settingChangeCount++;
			adjustCstickOffset(XAXIS, INCREASE, btn, hardware, controls);
		}
		else if (hardware.R && hardware.X && hardware.Z && hardware.Dd)
		{ // Decrease C-stick X Offset
			settingChangeCount++;
			adjustCstickOffset(XAXIS, DECREASE, btn, hardware, controls);
		}
		else if (hardware.R && hardware.Y && hardware.Z && hardware.Du)
		{ // Increase C-stick Y Offset
			settingChangeCount++;
			adjustCstickOffset(YAXIS, INCREASE, btn, hardware, controls);
		}
		else if (hardware.R && hardware.Y && hardware.Z && hardware.Dd)
		{ // Decrease C-stick Y Offset
			settingChangeCount++;
			adjustCstickOffset(YAXIS, DECREASE, btn, hardware, controls);
		}
		else if (hardware.R && hardware.A && hardware.Z && hardware.Du)
		{ // Increase C-stick Cardinal Snapping
			settingChangeCount++;
			adjustCardinalSnapping(CSTICK, INCREASE, btn, hardware, controls);
		}
		else if (hardware.R && hardware.A && hardware.Z && hardware.Dd)
		{ // Decrease C-stick Cardinal Snapping
			settingChangeCount++;
			adjustCardinalSnapping(CSTICK, DECREASE, btn, hardware, controls);
		}
		else if (hardware.L && hardware.A && hardware.Z && hardware.Du)
		{ // Increase C-stick Analog Scaler
			settingChangeCount++;
			adjustAnalogScaler(CSTICK, INCREASE, btn, hardware, controls);
		}
		else if (hardware.L && hardware.A && hardware.Z && hardware.Dd)
		{ // Decrease C-stick Analog Scaler
			settingChangeCount++;
			adjustAnalogScaler(CSTICK, DECREASE, btn, hardware, controls);
		}
		else if (hardware.R && hardware.S && !hardware.A && !hardware.L && !hardware.X && !hardware.Y)
		{ // Show Current C-stick Settings (ignore R jump and R trigger toggle and LRAS)
			showCstickSettings(btn, hardware, controls, gains);
		}
		else if (hardware.A && hardware.B && hardware.L)
		{ // Toggle Analog L
			settingChangeCount++;
			nextTriggerState(LTRIGGER, btn, hardware, controls);
		}
		else if (hardware.A && hardware.B && hardware.R)
		{ // Toggle Analog R
			settingChangeCount++;
			nextTriggerState(RTRIGGER, btn, hardware, controls);
		}
		else if (hardware.L && hardware.B && hardware.Du)
		{ // Increase L-Trigger Offset
			settingChangeCount++;
			adjustTriggerOffset(LTRIGGER, INCREASE, btn, hardware, controls);
		}
		else if (hardware.L && hardware.B && hardware.Dd)
		{ // Decrease L-trigger Offset
			settingChangeCount++;
			adjustTriggerOffset(LTRIGGER, DECREASE, btn, hardware, controls);
		}
		else if (hardware.R && hardware.B && hardware.Du)
		{ // Increase R-trigger Offset
			settingChangeCount++;
			adjustTriggerOffset(RTRIGGER, INCREASE, btn, hardware, controls);
		}
		else if (hardware.R && hardware.B && hardware.Dd)
		{ // Decrease R-trigger Offset
			settingChangeCount++;
			adjustTriggerOffset(RTRIGGER, DECREASE, btn, hardware, controls);
		}
		else if (hardware.X && hardware.Z && hardware.S)
		{ // Swap X and Z
			settingChangeCount++;
			setJumpConfig(SWAP_XZ, controls);
			freezeSticks(2000, btn, hardware);
		}
		else if (hardware.Y && hardware.Z && hardware.S)
		{ // Swap Y and Z
			settingChangeCount++;
			setJumpConfig(SWAP_YZ, controls);
			freezeSticks(2000, btn, hardware);
		}
		else if (hardware.X && hardware.L && hardware.S)
		{ // Swap X and L
			settingChangeCount++;
			setJumpConfig(SWAP_XL, controls);
			freezeSticks(2000, btn, hardware);
		}
		else if (hardware.Y && hardware.L && hardware.S)
		{ // Swap Y and L
			settingChangeCount++;
			setJumpConfig(SWAP_YL, controls);
			freezeSticks(2000, btn, hardware);
		}
		else if (hardware.X && hardware.R && hardware.S)
		{ // Swap X and R
			settingChangeCount++;
			setJumpConfig(SWAP_XR, controls);
			freezeSticks(2000, btn, hardware);
		}
		else if (hardware.Y && hardware.R && hardware.S)
		{ // Swap Y and R
			settingChangeCount++;
			setJumpConfig(SWAP_YR, controls);
			freezeSticks(2000, btn, hardware);
		}
		else if (checkAdjustExtra(EXTRAS_UP, btn, false))
		{ // Toggle Extras
			settingChangeCount++;
		...
		else if (checkAdjustExtra(EXTRAS_UP, btn, true))
		{ // Configure Extras
			settingChangeCount++;
	...
	else if (currentCalStep == -1)
	{ // Safe Mode Enabled, Lock Settings, wait for safe mode command
		...
		if (hardware.A && hardware.X && hardware.Y && hardware.S && !hardware.L && !hardware.R)
		{ // Safe Mode toggle
			if (safeModeLockout > 0)
			{ // Not held long enough
				safeModeLockout--;
			}
			else if (safeModeLockout == 0)
			{ // Held long enough
				safeModeLockout = safeModeLockoutDuration;
				if (!running)
				{ // wake it up if not already running
					running = true;
				}
...
void readSticks(int readA, int readC, Buttons &btn, Pins &pin, RawStick &raw, const Buttons &hardware, const ControlConfig &controls, const FilterGains &normGains, const StickParams &aStickParams, const StickParams &cStickParams, float &dT, int &currentCalStep)
{
	...
	*/
	...
	lastMicros += 1000;
	if (micros() - lastMicros > 1500)
	{ // handle the case that it was synced and now isn't
		lastMicros = micros();
	}
	raw.axRaw = aStickX;
	raw.ayRaw = aStickY;
